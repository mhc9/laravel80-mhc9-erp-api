...
class WpmCheckTimeController extends Controller
{
    ...
    public function getAll(Request $req)
    {
        ...
        $events = WpmCheckTime::with('employee')
                    // ->when(!empty($type), function($q) use ($type) {
                    //     $q->where('OTType', $type);
                    // })
    ...
    {
        return [
            'daily' => WpmCheckTime::with('employee')->whereRaw('CAST(CheTmDate AS DATE) = ?', [$date])->get(),
        ];
    }
    ...
    {
        try {
            $attendance = new WpmCheckTime();
            $attendance->CheTmEmId   = $req['employee_id'];
            $attendance->CheTmDate   = $req['check_in_date'];
            $attendance->CheTmPic    = $req['check_in_image'];
            $attendance->CheTmMark   = $req['remark'];
            $attendance->CheTmLastDate = Carbon::now();
            $attendance->CheTmType   = $req['check_type_id'] == '1' ? 'เข้า' : 'ออก';
